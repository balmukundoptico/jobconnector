...
import { getProfile, searchJobs, applyToJob } from '../utils/api'; // Adjust path as needed
...
export default function SeekerDashboard({ isDarkMode, toggleDarkMode, route }) {
  ...
  const handleApply = async (jobId) => {
    try {
      ...
      setAppliedJobs((prev) => [
        ...prev,
        { jobId, title: jobs.find((job) => job._id === jobId).jobTitle, status: 'Applied' },
      ]);
      setJobs((prev) =>
  ...
  const handleWhatsAppConnect = (number, jobTitle) => {
    ...
    setAppliedJobs((prev) => [
      ...prev,
      { jobId: jobTitle, title: jobTitle, status: 'Connected' },
    ]);
  ...
  const handleDownloadResume = async () => {
    ...
    try {
      ...
      if (Platform.OS === 'web') {
        ...
        // For mobile (Android/iOS), download directly using Linking
        const supported = await Linking.canOpenURL(resumeUrl);
        if (supported) {
          await Linking.openURL(resumeUrl);
          setMessage('Resume download initiated. Check your browser or downloads.');
          Alert.alert('Download Started', 'Check your browser or downloads folder.');
        } else {
          setMessage('Cannot open resume URL');
            ...
            <>
              ...
              <View
                style={[
                  ...
                  isDarkMode ? styles.darkProfileContainer : styles.lightProfileContainer,
                ]}
              >
                    ...
                    <View style={styles.jobActions}>
                      ...
                      >
                        <Animated.View
                          style={{
                            transform: [
                              { scale: applyScales[item._id]?.apply || new Animated.Value(1) },
                            ],
                          }}
                        >
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() =>
                          handleWhatsAppConnect(item.postedBy?.hrWhatsappNumber || '', item.jobTitle)
                        }
                      ...
                      >
                        <Animated.View
                          style={{
                            transform: [
                              { scale: connectScales[item._id]?.connect || new Animated.Value(1) },
                            ],
                          }}
                        >
