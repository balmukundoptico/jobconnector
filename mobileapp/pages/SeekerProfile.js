...
import { createSeekerProfile, updateSeekerProfile } from '../utils/api'; // Adjust path as needed
...
const SeekerProfile = ({ isDarkMode, toggleDarkMode, route }) => {
  ...
  const handleFilePick = async () => {
    try {
      ...
      if (!result.canceled && result.assets) {
        ...
        console.log('File data set:', fileData);
      } else {
  ...
  const handleSubmitProfile = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to save profile: ' + (error.response?.data?.message || error.message));
    } finally {
